...
export default function ChatBox({ docId, docName, onDocumentChange, existingNames }: ChatBoxProps) {
  ...
  const ingestYoutube = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/youtube_ingest`, { url });
    ...
    <div className="chat-box">
      {docName && mode === "rag" && (
        <p className="chat-empty" style={{ marginBottom: 4 }}>
          IntelX is answering with: <strong>{docName}</strong>
        </p>
      )}
      ...
      {mode === "youtube" && (
        <div className="chat-youtube-row">
          <div className="chat-youtube-field">
            <div className="chat-youtube-label">YouTube URL</div>
            <input
      ...
      <div className="chat-messages">
        {chat.length === 0 && (
          <p className="chat-empty">Ask IntelX a question about your uploaded content to get started.</p>
        )}
        {chat.map((m, i) => (
          <div
            key={i}
